 ...
 * @LastEditors: zbx
 * @LastEditTime: 2025-12-04 19:12:25
-->
<template>
    <div class="templateC">
        <div style="padding: 20px 0">
            <Form inline>
                <FormItem>
                    员工姓名:
                    <Input placeholder="请输入" style="width: 130px" clearable v-model="pageInfo.data.name" />
                </FormItem>
                <FormItem>
                    满意度:
                    <Select placeholder="请选择" style="width: 100px" v-model="pageInfo.data.comment" clearable>
                ...
                <FormItem>
                    发起人:
                    <Select v-model="pageInfo.data.senderId" placeholder="请选择" style="width: 130px" clearable
                        filterable>
                        <Option v-for="item in userList" :value="item.code" :key="item.code">{{ item.name }}</Option>
                    </Select>
                </FormItem>
                <FormItem>
                    时间范围:
                    <DatePicker type="daterange" split-panels placeholder="请选择时间范围" @on-change="dateSelect"
                        style="width: 220px" v-model="daterange" format="yyyy-MM-dd"></DatePicker>
                </FormItem>

                <FormItem>
                    <!-- accept="image/png, image/jpeg"  -->
                    <Upload action="image/upload" :before-upload="beforeUpload" multiple>
                        <Button style="width: 150px" type="primary">点击上传</Button>
                        <p>{{ fileObj.fileName }}</p>
                    </Upload>
                </FormItem>

                <Button type="primary" icon="md-search" class="ml10" @click="lookUp">查询</Button>
                <Button class="ml10" @click="reset">重置</Button>
                <Button type="warning" class="ml10" @click="toAdd">新增</Button>
                <Button type="success" class="ml10" @click="exportExcel" :loading="exportLoading">导出</Button>
                <Button type="error" class="ml10" @click="regTest">正则</Button>
                <Button type="error" class="ml10 test" @click="htmlCapture">转图片</Button>
                <Button class="ml10" @click="getApi">getApi</Button>
                <Button class="ml10" @click="getFileList">getFileList</Button>
            </Form>
        </div>
        <Table :data="tableList" :columns="tableColumn" :loading="tableLoading" stripe border max-height="750">
            <template slot="operate" slot-scope="{ row }">
                <Button type="warning" @click="toEdit(row)" icon="md-create"></Button>
                <Button type="error" @click="del(row)" class="ml10" icon="ios-trash-outline"></Button>
            </template>
        </Table>
        <div style="padding: 20px 0; text-align: right">
            <Page :current="pageInfo.pageNum" :page-size="pageInfo.pageSize" :total="pageInfo.total" show-elevator
                show-total show-sizer :page-size-opts="[10, 20, 30, 40, 50]" @on-change="changePage"
                @on-page-size-change="changePageSize">
            </Page>
        </div>

        <Modal v-model="showPageModal" :title="pageForm.id ? '编辑' : '新增'" :mask-closable="false" class="modalClass">
            <Form :model="pageForm" :rules="pageRule" :label-width="120" ref="pageForm">
                <FormItem prop="name" label="名称">
                    <Input v-model="pageForm.name" placeholder="请输入" style="width: 300px" />
                </FormItem>
                <FormItem prop="country" label="国家">
                    <Input v-model="pageForm.country" placeholder="请输入" style="width: 300px" />
                </FormItem>
                <FormItem prop="alexa" label="alexa">
                    <Input v-model="pageForm.alexa" placeholder="请输入" style="width: 300px" />
                </FormItem>
                ...
                <FormItem prop="url" label="备注">
                    <Input v-model="pageForm.url" maxlength="300" :rows="4" show-word-limit type="textarea"
                        placeholder="简要说明" style="width: 300px" />
                </FormItem>
            </Form>
        ...
        <div class="imgListCon">
            <img :src="'http://localhost:8094/nodeApi/test/getFile/?id=' + item.id" :alt="item.filename"
                v-for="item in fileList" :key="item.id" style="width: 200px;height:150px">
        </div>
        <div style="font-size: 40px;">{{ localCount }}</div>
    </div>
</template>
<script>
